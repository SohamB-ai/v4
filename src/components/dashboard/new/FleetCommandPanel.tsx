...
const healthData = [
    { name: "Healthy", value: 65, color: "#00E0C6" }, // Neon Teal
    { name: "Warning", value: 25, color: "#FFB020" }, // Amber
    { name: "Critical", value: 10, color: "#FF5C5C" }, // Coral Red
];
        ...
        <div className="h-full flex flex-col gap-6">
            ...
            <div className="bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 p-5 font-serif font-tinos">
                <h3 className="text-white/90 font-semibold mb-4 flex items-center gap-2">
                    <Activity className="w-4 h-4 text-[#00E0C6]" /> Fleet Health
                </h3>
                <div className="h-40 w-full relative">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                        <div className="text-center">
                            <span className="block text-2xl font-bold text-white font-tinos">142</span>
                            <span className="text-[10px] text-slate-400 uppercase tracking-widest font-tinos font-inter">Total</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            {/* Risk Heatmap List */}
            <div className="bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 p-5 flex-1 flex flex-col min-h-0 font-tinos font-serif">
                <h3 className="text-white/90 font-semibold mb-3 flex items-center gap-2 font-tinos">
                ...
                <div className="space-y-2 overflow-y-auto scrollbar-thin scrollbar-thumb-white/10 pr-2">
                    {riskList.map((asset) => (
                        ...
                        >
                            <div>
                                <div className="font-mono text-sm font-bold text-white">{asset.id}</div>
                            ...
                            <div className="text-right">
                                <div className="text-white font-bold">{asset.score}%</div>
                ...
                <div className="space-y-3 font-tinos">
                    {degradingAssets.map((asset) => (
                        <div key={asset.id} className="flex items-center justify-between group cursor-pointer font-tinos">
                            <span className="text-slate-300 font-medium text-sm group-hover:text-white transition-colors">{asset.id}</span>
                            <div className="flex items-center gap-2">
                                <span className="text-sm font-bold text-white font-mono">HI {asset.hi}</span>
            ...
            <div className="bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 p-5 font-tinos font-serif">
                ...
                <div className="relative border-l border-white/10 ml-2 space-y-6 pl-4 py-1">
                    {alerts.map((alert, i) => (
                        <div key={i} className="relative">
                            <div className={cn("absolute -left-[21px] top-1 w-2.5 h-2.5 rounded-full ring-4 ring-black",
                                alert.type === "critical" ? "bg-[#FF5C5C]" :
                                    alert.type === "warning" ? "bg-[#FFB020]" : "bg-cyan-400"
                            )} />
                            <div className="text-xs text-slate-400 font-mono mb-0.5">{alert.time}</div>
                            <div className="text-sm text-white font-medium">{alert.msg}</div>
                        </div>
                    ))}
                </div>
            </div>
